...
mod tests {
    ...
    #[test]
    fn test_entity_entry_new() {
        let entry = EntityEntry::new("light.living_room", "hue", Some("unique123".to_string()));
        assert_eq!(entry.entity_id, "light.living_room");
        assert_eq!(entry.platform, "hue");
        assert_eq!(entry.unique_id, Some("unique123".to_string()));
    ...
    fn test_get_or_create_new() {
        ...
        assert_eq!(entry.entity_id, "light.living_room");
        assert_eq!(entry.unique_id, Some("unique123".to_string()));
        assert_eq!(entry.config_entry_id, Some("config1".to_string()));
        assert_eq!(entry.device_id, Some("device1".to_string()));
        assert_eq!(registry.len(), 1);
    }

    #[test]
    fn test_get_or_create_existing_unique_id() {
        let (_dir, registry) = create_test_registry();

        // Create first entity
        registry.get_or_create("hue", "light.test1", Some("unique123"), None, None);

        // Try to create with same unique_id
        let entry = registry.get_or_create("hue", "light.test2", Some("unique123"), None, None);

        // Should return existing
        assert_eq!(entry.entity_id, "light.test1");
        assert_eq!(registry.len(), 1);
    }
    ...
    fn test_indexes() {
        ...
        // Test by_unique_id
        let entry = registry.get_by_unique_id("light1").unwrap();
        assert_eq!(entry.entity_id, "light.living_room");

        // Test by_device_id
        let device1_entities = registry.get_by_device_id("device1");
        assert_eq!(device1_entities.len(), 2);

        // Test by_config_entry_id
        let config1_entities = registry.get_by_config_entry_id("config1");
        assert_eq!(config1_entities.len(), 2);

        // Test by_platform
        let hue_entities = registry.get_by_platform("hue");
        assert_eq!(hue_entities.len(), 2);
    }
    ...
    fn test_update() {
        ...
        registry.get_or_create("hue", "light.test", Some("unique1"), None, None);

        let updated = registry.update("light.test", |entry| {
            entry.name = Some("My Light".to_string());
            entry.device_id = Some("device1".to_string());
        });

        assert_eq!(updated.name, Some("My Light".to_string()));
        assert_eq!(updated.device_id, Some("device1".to_string()));

        // Verify indexes updated
        let by_device = registry.get_by_device_id("device1");
    ...
    fn test_remove() {
        ...
        registry.get_or_create("hue", "light.test", Some("unique1"), None, Some("device1"));

        let removed = registry.remove("light.test").unwrap();
        assert_eq!(removed.entity_id, "light.test");

        // Should be gone from all indexes
        assert!(registry.get("light.test").is_none());
        assert!(registry.get_by_unique_id("unique1").is_none());
        assert!(registry.get_by_device_id("device1").is_empty());
    }
    ...
    async fn test_save_and_load() {
        ...
        // Load into new registry
        {
            ...
            assert_eq!(registry.len(), 1);
            let entry = registry.get("light.test").unwrap();
            assert_eq!(entry.unique_id, Some("unique1".to_string()));
        }
    }

    #[test]
    fn test_entity_disabled_hidden() {
        ...
        entry.hidden_by = Some(HiddenBy::Integration);
        assert!(entry.is_hidden());
    }
}
