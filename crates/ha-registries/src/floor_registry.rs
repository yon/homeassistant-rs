...
mod tests {
    ...
    fn create_test_registry() -> (TempDir, FloorRegistry) {
    ...
    fn test_create_floor() {
        ...
        let floor = registry.create("Ground Floor", 0);
    ...
    fn test_get_by_name() {
        ...
        registry.create("Ground Floor", 0);

        let floor = registry.get_by_name("ground floor").unwrap();
        assert_eq!(floor.name, "Ground Floor");
    }
    ...
    fn test_get_by_level() {
        ...
        registry.create("Ground Floor", 0);
        registry.create("First Floor", 1);
        registry.create("Basement", -1);

        assert!(registry.get_by_level(0).is_some());
        assert!(registry.get_by_level(1).is_some());
        assert!(registry.get_by_level(-1).is_some());
        assert!(registry.get_by_level(2).is_none());
    }

    #[test]
    fn test_sorted_by_level() {
        let (_dir, registry) = create_test_registry();
        ...
        assert_eq!(sorted[1].level, 0);
        assert_eq!(sorted[2].level, 1);
    }
}
